...
#### Task & Subtask Numbering Format (MANDATORY)

**任务标号**：`<章节号>.<序号>`，如 `1.1`、`2.1`、`3.1`

**子任务标号**：`<父任务标号>.<子序号>`，如 `1.1.1`、`1.1.2`、`3.1.1`、`3.1.2`

**所有子任务都需要有标号**：无论是状态、切换还是 Case，其下的每个子任务都必须有标号：
- **Chapter 2 状态/切换**：每个子任务使用标号如 `2.1.1`、`2.1.2`
- **Chapter 3 Case**：每个子任务使用标号如 `3.1.1`、`3.1.2`

**步骤格式**：步骤不再需要序号，直接描述操作即可
```
- [ ] 1.1 档位默认状态
  - [ ] 1.1.1 读取当前档位状态
...
- [ ] 2.1 状态: heron-selected
  - [ ] 2.1.1 启动应用并进入船只建造视图
  - [ ] 2.1.2 选择 class=L, race=teladi, type=freighter
  - [ ] 2.1.3 选择 Heron Vanguard
  - [ ] 2.1.4 断言属性面板可见
...
**错误格式**:
- ❌ `- [ ] 步骤 1: 读取当前档位状态` (不需要"步骤 1"序号)
- ❌ `- [ ] 1.1 任务` 下直接是 `- [ ] 步骤 1` (子任务需要标号 1.1.1)
- ❌ `- [ ] 2.1 状态: xxx` 下直接是 `- [ ] 前提: ...` (状态子任务需要标号 2.1.1)
- ❌ `- [ ] 3.1 Case: xxx` 下直接是 `- [ ] 点击按钮` (Case子任务需要标号 3.1.1)
...
1. **每个 x.x 任务必须至少有一个子任务**
   - 格式：`- [ ] <父任务标号>.<子序号> <description>`
   - 子任务标号从 `.1` 开始递增，如 `1.1.1`、`1.1.2`、`1.1.3`
2. **每个 x.x 任务的最后一个子任务必须包含”期望”**
3. **所有包含”期望”的子任务必须内联断言方法**
   - 例如：`expect(...)`、`toBe(...)`、`toEqual(...)`、`toContain(...)`
4. **步骤不再要求有序号**
   - 直接描述操作，如：`- [ ] 1.1.1 读取当前档位状态`
   - 不再使用 `- [ ] 步骤 1:` 格式
5. **不允许以下格式**
   - `- 步骤 <n>: ...`（缺少 checkbox）
   - `### 步骤 <n>`（标题式步骤）
6. **Chapter 3 Case 子任务缩进规则**
   - 在 `Case` 下方、与步骤同为两格缩进的条目都视为 Case 子任务
   - 这些条目必须使用 checkbox：`- [ ] ...`（包括 `前提:` / `前提补充:` 等）
7. **Chapter 2 粒度规则（防止状态/切换泛滥）**
   - `状态:` 任务至少包含 4 条子任务
   - `切换:` 任务至少包含 3 条子任务
