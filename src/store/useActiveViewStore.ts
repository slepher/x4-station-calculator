...
export interface ActiveViewState {
  ...
  activeBindingWorkbench: 'overview' | 'station' | 'transit' | 'npc-trade' | 'terraforming' | 'tech-tree' | 'research' | 'blueprint-recipe' | 'auto-sector-group'
...
export const useActiveViewStore = defineStore('activeView', () => {
  ...
  const activeBindingStation = computed({
    ...
    set: (val: string | null) => {
      ...
      const FIXED_MODES = ['npc-trade', 'terraforming', 'tech-tree', 'research', 'blueprint-recipe', 'auto-sector-group']
