import { SAXParser, SaxEventType, type Tag as SaxWasmTag, type Attribute as SaxWasmAttribute } from 'sax-wasm'
import type {
  SaveArchive,
  SaveParserConfig,
  SaveParserMessage,
...
export const SAVE_PARSER_WASM_URL = new URL('../../node_modules/sax-wasm/lib/sax-wasm.wasm', import.meta.url)

function stripSaveFileExtension(filename: string): string {
  return filename.replace(/(\.xml)?\.gz$/i, '').replace(/\.xml$/i, '')
}

function attributeListToRecord(attributes: readonly SaxWasmAttribute[]): Record<string, unknown> {
  const result: Record<string, unknown> = {}
  for (const attribute of attributes) {
    result[attribute.name.value.toLowerCase()] = attribute.value.value
  }
  return result
}

function tagToNode(tag: SaxWasmTag): TagNode {
  return {
    name: tag.name.toLowerCase(),
    attributes: attributeListToRecord(tag.attributes)
  }
}
...
export function createSaveParserRuntime(
  ...
  options?: {
    ...
    profile?: boolean
    wasmSource?: Uint8Array | WebAssembly.Module | Response | Promise<Response>
  }
  ...
  saxParser.eventHandler = (event, detail) => {
    if (event === SaxEventType.OpenTag) {
      const node = tagToNode(detail as SaxWasmTag)
      if (!profileEnabled) {
    ...
    if (event === SaxEventType.CloseTag) {
      const name = (detail as SaxWasmTag).name.toLowerCase()
      if (!profileEnabled) {
  ...
  const ready = saxParser.prepareWasm(options?.wasmSource ?? fetch(SAVE_PARSER_WASM_URL))

  return ready.then(() => ({
    feed(input: Uint8Array | string) {
      if (!input) return
...
export async function parseSaveXmlChunks(
  runtime: SaveParserRuntime | Promise<SaveParserRuntime>,
  chunks: Iterable<string | Uint8Array> | AsyncIterable<string | Uint8Array>
): Promise<SaveArchive> {
  const resolvedRuntime = await runtime
  for await (const chunk of chunks) {
    resolvedRuntime.feed(chunk)
  }
  return resolvedRuntime.close()
}
