  ...
  <div class="x4-search-container" @mouseenter="isHovered = true" @mouseleave="isHovered = false">
    <div class="search-box" :class="{ 'focused': isFocused }">
      ...
      <input
        ...
        :value="modelValue" 
        class="search-input"
        :placeholder="placeholder || '搜索 (名称/ID/拼音)...'"
        @input="onInput"
        @focus="onFocus"
        @blur="onBlur"
        @keydown.esc="onEsc"
    ...
    <Transition name="fade-slide">
      <div v-if="isFocused" class="results-popover scrollbar-thin" @mousedown.prevent>
        <div v-for="(modules, type) in store.filteredModulesGrouped" :key="type" class="type-group">
          <div class="group-header">{{ type }}</div>
          <div v-for="m in modules" :key="m.id" class="result-item" @click="handleSelect(m)">
...
.results-popover { @apply absolute bottom-full mb-2 w-full bg-slate-900 border border-slate-700 rounded shadow-2xl z-50 max-h-80 overflow-y-auto; }
