...
const tooltipLines = computed(() => {
  ...
  return filteredLines.map(line => ({
    ...line,
    highlight: line.active && showHighlight
  }))
})
</script>
...
<template>
  <tippy theme="x4" :hide-on-click="false" interactive placement="left">
    ...
    <template #content>
      <div class="priority-tooltip-container">
        <div 
          v-for="line in tooltipLines" 
          :key="line.level" 
          class="priority-tooltip-row" 
          :class="{ 'is-active': line.highlight }"
        >
          <span class="icon-cell">
            <svg v-if="line.level === 0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-3.5 h-3.5 align-middle mr-1">
              <path stroke-linecap="round" stroke-linejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
            </svg>
            <svg v-else-if="line.level === 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-3.5 h-3.5 align-middle mr-1">
              <defs>
                <linearGradient :id="'tipHalfStar' + line.level" x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              <path :fill="'url(#tipHalfStar' + line.level + ')'" stroke="none" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
            </svg>
