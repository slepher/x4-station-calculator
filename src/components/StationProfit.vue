...
<style scoped>
.panel-container {
  @apply bg-slate-800 rounded border border-slate-700 overflow-hidden text-sm mt-4 shadow-2xl;
}
...
.simulation-controls {
  @apply p-4 bg-slate-900/50 border-b border-slate-700 flex flex-row;
}

/* 基础结构 */
.section-header {
  @apply flex justify-between px-3 py-2 bg-slate-800/50 border-b border-slate-700/50 cursor-pointer hover:bg-slate-700/50 transition-colors select-none;
}

.section-label {
  @apply flex items-center gap-2 font-bold text-[11px] uppercase;
}
...
.section-expenses .total-val {
  @apply text-red-400;
}

.section-expenses .arrow-open {
  @apply text-red-400;
}

.section-expenses .list-item .item-name {
  @apply text-red-400/70;
}

.section-expenses .list-item .qty {
  @apply text-red-400/80;
}

.section-expenses .list-item .item-val {
  @apply font-mono font-medium text-red-500;
}

/* --- 产出模块色彩封装 --- */
.section-production .section-label,
.section-production .total-val {
  @apply text-sky-400;
}

.section-production .arrow-open {
  @apply text-sky-400;
}

.section-production .list-item .item-name {
  @apply text-sky-400/70;
}

.section-production .list-item .qty {
  @apply text-sky-400/80;
}

.section-production .list-item .item-val {
  @apply font-mono font-medium text-sky-500;
}
