...
test.describe('Task 3: 统一资源仪表盘和视图模式切换测试', () => {
  ...
  test('3.1 视图模式切换功能测试', async ({ page }) => {
    ...
    // 验证按钮内容（使用翻译后的文本）
    await expect(quantityView).toHaveText(/数量视图|Quantity View/);
    await expect(economyView).toHaveText(/经济视图|Economy View/);
    await expect(volumeView).toHaveText(/体积视图|Volume View|仓储视图/);
    ...
    await expect(title).toContainText(/资源产出|Resource Production/);
    ...
    // 验证标题更新为"经济分析"
    await expect(title).toContainText(/经济分析|Economic Overview|Profit Analysis|利润分析|Economic/);
  ...
  test('3.5 国际化支持测试', async ({ page }) => {
    ...
    const dashboard = page.locator('.list-wrapper').first();
    const title = dashboard.locator('.header-title');
    await expect(title).toContainText(/Resource Production/);
