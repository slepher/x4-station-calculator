...
describe('useMapSvgOverlays save POI culling', () => {
  ...
  it('keeps large icons at half cluster up to scale 0.5, then smoothly shrinks to 18xscale on screen at max scale', () => {
    const clusters = computed(() => ({
      cluster_01: {
        id: 'cluster_01',
        normalized: { pixel_basis: { x: 0, y: 0 } },
    ...
    expect(midSize).toBeGreaterThan(18)
    expect((maxScaleSize || 0) * 1.2).toBeCloseTo(18 * 1.2, 1)
  })
})
