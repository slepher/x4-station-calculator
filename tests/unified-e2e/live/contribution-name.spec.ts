...
test.describe('Contribution name 显示验证', () => {
  ...
  test('新建空间站缺口: 星区运营量子管明细应含 "阿尔忒弥斯的朦胧" 和 "警惕凝视"', async ({ page }) => {
    ...
    const stationDashboard = page.locator('[data-testid="station-dashboard"]')
    await expect(stationDashboard).toBeVisible({ timeout: 2000 })

    const sectorOpsSection = stationDashboard.locator('.empire-gap-group').filter({ hasText: /星区运营|Sector Operations/i })
