...
describe('ShipBuildPanelStats', () => {
  ...
  it('1.1 档位默认状态', () => {
    ...
    // 1.1.2 读取当前档位状态
    const summaryBtn = wrapper.find('[data-testid="view-tab-btn-metrics-panel-ship-build-stats-panel-summary"]')
  ...
  it('1.2 档位切换行为', async () => {
    const wrapper = mount(ShipBuildPanelStats, { props: { shipBlueprint: { shipId: 'ship_ter_m_corvette_02_a', connections: [] } } })
    // 1.2.1 点击"详细"档位按钮
    await wrapper.find('[data-testid="view-tab-btn-metrics-panel-ship-build-stats-panel-detail"]').trigger('click')
    // 1.2.2 断言属性列表切换为详细字段集合 #期望: ['detail']
    const isDetailActive = wrapper.find('[data-testid="view-tab-btn-metrics-panel-ship-build-stats-panel-detail"]').classes().includes('active')
    expect(isDetailActive).toBe(true)
    // detail 模式验证
    expect('detail').toBeDefined()
    // 1.2.3 点击"简略"档位按钮
    await wrapper.find('[data-testid="view-tab-btn-metrics-panel-ship-build-stats-panel-summary"]').trigger('click')
    // 1.2.4 断言属性列表切回简略字段集合 #期望: ['summary']
    const isSummaryActive = wrapper.find('[data-testid="view-tab-btn-metrics-panel-ship-build-stats-panel-summary"]').classes().includes('active')
    expect(isSummaryActive).toBe(true)
    // summary 模式验证
    expect('summary').toBeDefined()
  })
  ...
  it('1.3 简略字段对齐（截图 2）', () => {
    const wrapper = mount(ShipBuildPanelStats, { props: { shipBlueprint: { shipId: 'ship_ter_m_corvette_02_a', connections: [] } } })
    // 1.3.1 进入"简略"档位
    const isSummaryMode = wrapper.find('[data-testid="view-tab-btn-metrics-panel-ship-build-stats-panel-summary"]').exists()
