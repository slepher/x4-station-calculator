...
test.describe('simplify-flow bug', () => {
  ...
  test('4.1 BUG-001: V2 节点加载后仍保留旧字段导致 V3 迁移不完整', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      flow.version = 2
      flow.list[0].groups[0].nodes[0] = {
        id: 'legacy-node-1',
        wareId: 'claytronics',
        moduleId: 'module_gen_prod_claytronics_01',
        race: 'default',
        lineage: 'default',
        column: 3,
        isIsolated: false,
    ...
    // 4.1.4 修复前断言 `x4_logic_flow_plans.list[0].groups[0].nodes[0]` 仍包含 `moduleId` 字段 #期望: [true]
    const hasLegacyModuleId = await page.evaluate(() => {
      const flow = JSON.parse(localStorage.getItem('x4_logic_flow_plans') || '{}')
      const node = flow.list?.[0]?.groups?.[0]?.nodes?.[0] || {}
      return Object.keys(node).includes('moduleId')
    })
    expect(hasLegacyModuleId).toBe(true)
  ...
  test('4.2 BUG-002: Empire 导入 flow 时忽略 isolated 节点的锁定货物映射', async ({ page }) => {
    ...
    // 4.2.4 修复前断言导入目标站点 `lockedWares` 不包含 `quantumtubes` #期望: [false]
    const hasQuantumBefore = await page.evaluate(() => {
    ...
    expect(hasQuantumBefore).toBe(false)
    ...
    expect(Array.isArray(lockedWares)).toBe(true)
