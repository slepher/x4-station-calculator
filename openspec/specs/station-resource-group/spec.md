...
## Purpose
TBD - created by archiving change station-resource-group. Update Purpose after archive.
## Requirements
### Requirement: Resource Group Loader Component

资源高级筛选器顶部 SHALL 新增载入组件，允许用户从星区或逻辑组网载入资源组配置。
...
载入组件 SHALL 从现有数据源获取星区、空间站和逻辑组网存档信息。
...
刷新按钮 SHALL 与 pending 提示共享父元素，无待刷新时 SHALL 隐藏。
