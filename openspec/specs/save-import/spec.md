...
## Purpose
TBD - created by archiving change rebase-sector-zeropoint. Update Purpose after archive.
## Requirements
...
新增"存档同步"Tab入口。 SHALL pass validation
...
提供上传界面，支持原始存档和已提取JSON两种格式。 SHALL pass validation
...
存档必须同时记录 parser 原始数据版本和后处理版本。 SHALL pass validation
...
存档版本必须与当前游戏数据版本匹配，版本不匹配时拒绝加载。 SHALL pass validation
...
存档按guid分组，按time降序排列。 SHALL pass validation
...
IndexedDB 中的缓存存档必须按版本规则恢复，而不是一律删除。 SHALL pass validation
...
无效存档必须显式展示，但不能继续进入依赖其内容数据的深层交互。 SHALL pass validation
...
从存档XML提取指定对象。 SHALL pass validation
...
提取对象按sector组织。 SHALL pass validation
...
JS parser 冻结为兼容/备用链路，后续业务演进仅进入 Rust/WASM 解析链。 SHALL pass validation
...
右侧面板展示存档详情。 SHALL pass validation
...
支持导出提取结果为JSON文件。 SHALL pass validation
...
存档解析使用流式解析避免阻塞UI。 SHALL pass validation
...
存档内 `{page,id}` 格式名称需翻译。 SHALL pass validation
...
新增 `useSaveStore` 管理存档数据。 SHALL pass validation
...
提供命令行工具进行存档提取。 SHALL pass validation
