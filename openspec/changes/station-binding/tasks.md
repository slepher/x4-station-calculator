...
- [x] 23.1 调整 Step 2 收缩态 empire sector 条目交互：主体不再承担点击展开，改为明确的“编辑”按钮进入展开态
- [x] 23.2 调整 Step 2 收缩态布局：标题区不再显示定位星区药丸，正文同一行显示定位星区药丸与 Step 3 按钮
- [x] 23.3 增加单星区编辑模式约束：存在展开态星区时，所有排序手柄与 Step 3 按钮失效，且通过“确定 / 取消”退出展开态
- [x] 23.4 将“创建星区”入口放到 Step 2 收缩态列表层，并改为先打开新建 draft，确认时才真正创建 empire sector
- [x] 23.5 在 Step 2 展开编辑态标题行支持修改单个 empire sector 名字，并将 `绑定星区>` 按钮放在同一行右侧
- [x] 23.6 为 `GroupSaveBinding` 增加连接星区持久化字段，存储双向 empire sector 连接关系，替代未来将废弃的旧 empire-level 连接语义
- [x] 23.7 基于定位星区实现 5 跳内 empire sector 候选搜索；未绑定定位星区的 empire sector 不参与连接候选计算
- [x] 23.8 调整 Step 2 展开编辑态“定位星区”段落：标题样式保持不变，内容改为“定位星区药丸 + 跳数控件”，移除跳数描述文字
- [x] 23.9 调整“绑定星区”菜单：同时高亮 store 定位星区与 draft 定位星区，draft 定位星区不可点击，移除解绑 `x`
- [x] 23.10 在 Step 2 展开编辑态新增“连接星区”区块，按跳数分组显示绿色已连接与红色未连接药丸，并支持 `x` 取消 / `+` 连接
- [x] 23.11 调整连接星区药丸文本为 `<empire sector>:<map sector>`
- [x] 23.12 调整 Step 2 收缩态结果展示：连接星区并入按跳数分组的正文结果中，不再独立显示“连接星区”字样，且颜色与编辑态保持一致
- [x] 23.13 增加展开态底部操作区：取消 / 确认移动到底部，已有星区显示红色删除按钮，新建 draft 不显示删除按钮
- [x] 23.14 实现删除确认浮层/行内确认，不使用系统 `alert/confirm`
- [x] 23.15 实现删除已有 empire sector 的级联清理：删除 `groupBinding`、清理双向连接、删除 empire sector、清理 empire station 对其的引用
- [x] 23.16 实现连接关系双向同步：A 与 B 建立或取消连接时，双方 `groupBinding` 的连接数据同时更新
- [x] 23.17 调整 empire sector 下属 map 星区药丸样式，为药丸之间增加可读间距
- [x] 23.18 调整下方存档星区列表摘要：显示星区数量，不再显示坐标点数量
- [x] 23.19 调整下方存档星区的空间站展示：名称来源改为与 save tooltip 标题一致，不再回退显示 `code`
- [x] 23.20 调整下方存档星区的空间站展示：按同名聚合、数量大于 1 时显示 `xN`
- [x] 23.21 调整下方存档星区列表行为：不再承担进入 Step 3 的职责，仅保留参考与定位信息
