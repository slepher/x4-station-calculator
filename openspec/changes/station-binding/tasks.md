...
- [x] 6.2 支持将空闲 empire station 直接拖拽到地图
- [x] 6.3 拖拽后按小空间站尺寸渲染
- [x] 6.4 将拖拽得到的 `position: { x, y, z }` 只写入 binding，不写入 `EmpirePlan`
...
- [x] 8.2 提供 `SaveBindingPlan` 选择与 `selectedArchiveTime` 切换入口
- [x] 8.3 实现"两段式"流程：save 星区列表 -> 跳数过滤结果
- [x] 8.4 进入第二段后自动缩放/平移地图到过滤星区最大范围
- [x] 8.5 为当前帝国星区展示绑定已有站、导入新站、绑定中转与拖拽空闲站入口

- [x] 9. 列表与操作区 UI
- [x] 9.1 第一段显示 save 星区列表
- [x] 9.2 第二段显示过滤后的 save 星区与空间站列表
- [x] 9.3 底部显示空闲 empire station 列表
- [x] 9.4 提供解绑与"当前 time 下失效"提示

- [x] 10. 构建验证
- [x] 10.1 完成实现后执行 `npm run build`
- [x] 10.2 若构建失败，修复后重新构建直至通过或记录 blocker