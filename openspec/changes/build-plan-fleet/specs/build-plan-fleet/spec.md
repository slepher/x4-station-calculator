...
为 build-plan 约束面板引入 Fleet goal，允许用户基于已保存的飞船蓝图配方设定持续造船目标，将蓝图材料需求按建造时间展开为 production-rate 进入现有 preview/compute 管线。
...
**那么** BuildGoal MUST 新增分支 `{ type: 'fleet'; buildTime: number; entries: FleetEntry[] }`
**并且** FleetEntry MUST 包含 `shipId`、`blueprintId`、`quantity`
**并且** MUST 删除旧的 `{ type: 'fleet'; shipId: string; quantity: number }` 分支
**并且** `buildTime` 默认 MUST 为 3600
**并且** `buildTime` 最小 MUST 为 600

#### Scenario: 创建 Fleet goal

**前提** buildGoals 中无 fleet goal
**当** 用户通过搜索添加第一个蓝图配方
**那么** 系统创建 `{ type: 'fleet', buildTime: 3600, entries: [{ shipId, blueprintId, quantity: 1 }] }`
...
### Requirement: Fleet 搜索框

**前提** 用户在 build-plan 约束面板的搜索框中
**当** 用户选择 fleet 类别
**那么** 系统 MUST 渲染独立的 FleetGoalSearchBox 组件
**并且** 搜索框 MUST 右侧弹出搜索结果
**并且** 搜索行为 MUST 根据 i18n 搜索舰船型号
**并且** 搜索结果 group MUST 为舰船型号，item MUST 为已保存蓝图
**并且** 搜索结果中 MUST NOT 显示无已保存蓝图的舰船
**并且** MUST NOT 显示 built-in preset 蓝图
**并且** 舰船分组 MUST 按 class 排序（ship_s → ship_m → ship_l → ship_xl），同 class 内按名称排序
...
**并且** Fleet 标题栏 MUST 显示固定文案 "Fleet" 和可编辑的 buildTime
...
**前提** Fleet goal 存在
**当** 约束面板渲染
**那么** 标题栏显示 "Fleet ⏱ {buildTime}s"
**并且** buildTime 可编辑，最小值 600
...
**并且** 收起时 MUST 显示 `舰船名 / 蓝图名 × quantity [× 删除]`
**并且** 点击条目 MUST 展开显示材料明细
...
**前提** 条目处于收起状态
**当** 用户点击条目
**那么** 条目展开显示该蓝图配方在当前 quantity 下的材料明细
...
**并且** 每种 ware 的 rate MUST 按 `Math.ceil(所有 entry 的同 wareId 总量之和 / buildTime × 3600)` 计算
...
**当** buildTime = 7200
**那么** hullparts rate = Math.ceil((950 + 4433) / 7200 × 3600) = 2692/h

### Requirement: Fleet 派生 rate 进入 preview/compute 管线

**前提** Fleet goal 存在
**当** 系统执行 createBuildFlowPlanPreview
**那么** Fleet 派生的 rate MUST 展开为 production-rate 子目标
...
**并且** FleetGoalView MUST 包含完整的 entries（含材料明细、蓝图名称、舰船名称）和 mergedRates
