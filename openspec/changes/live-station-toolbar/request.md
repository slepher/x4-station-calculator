...
```
[名称(可编辑)] [编码(只读)] [实时|规划 切换按钮] | [星区资源(只读)] [光伏效率(只读)] [单位吞吐量(只读)] | [偏好种族] [工人运算] [显示缺口] ← 仅规划模式显示
```
...
| 光伏效率 | station 所在 `sectorMacro` 对应的 map sector 的 `sunlight` 值 |
...
1. Toolbar 布局符合新结构：名称 + 编码 + 切换按钮 | 星区资源 + 光伏效率 + 单位吞吐量 | 规划专属字段
2. 模式切换按钮根据数据状态正确初始化和启用/禁用
3. 编码字段正确展示存档 station 的 code
4. 星区资源 popover 展示存档 sector 的 resources 列表（只读，无 checkbox）
5. 光伏效率展示存档 sector 的 sunlight 值（只读，无输入控件）
6. 规划模式下偏好种族、工人运算、显示缺口正常显示并可编辑
7. 实时模式下偏好种族、工人运算、显示缺口隐藏
8. 切换按钮点击可正确切换模式状态
9. TypeScript 编译无错误
10. `npm run build` 成功
