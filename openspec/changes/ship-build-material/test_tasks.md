...
- [ ] 2.3 切换：method 变更
  ...
  - [ ] 当前执行结果（2026-02-25）：失败。`ship-build-material-method-select` 中未出现 `terran` 选项（产品缺陷阻塞）。

- [x] 2.4 状态：标准测试状态-大阪-多模块聚合
  - [ ] 步骤 1：先进入“状态：标准测试状态-大阪”。
  - [ ] 步骤 2：在配装区切换到 `slotType=turret`。
...
- [ ] 3.4 场景：method fallback 生效
  ...
  - [ ] 当前执行结果（2026-02-25）：失败。因 `terran` method 选项缺失，步骤 4 无法执行（产品缺陷阻塞）。
