...
- [ ] 3.3b 场景：有 activeId 时默认载入对应 blueprint
  - 步骤 1：localStorage 包含 blueprint 数据，activeId 指向特定 blueprint。
  - 步骤 2：进入飞船建造视图。
  - 步骤 3：断言自动选中 activeId 对应的飞船。
  - 步骤 4：断言筛选区自动选中对应条件。
  - 步骤 5：断言配装区显示已保存的装备配置。
...
- [ ] 3.11 场景：切换船只需要清空装备
  - 步骤 1：进入”状态：持久化-已配置装备”。
  - 步骤 2：点击”更换飞船”按钮。
  - 步骤 3：在列表中选择另一艘飞船（如 Osaka）。
  - 步骤 4：断言原飞船的装备配置已清空（blueprint 为空或不含原装备）。
  - 步骤 5：断言新飞船的配装区为初始状态。

- [ ] 3.12 Bug修复验证 BUG-SBS-001（点击shield标签切换时无反应）
...
- [ ] 3.13 Bug修复验证 BUG-SBS-002（新建方案未正确重置状态）
  - **Bug现状**: 点击新建按钮，在 SmartSaveDialog 中选择”覆盖并新建”后，页面仍然保持原来的飞船选择和蓝图配置。
  - 步骤 1：进入ship-build，选择一艘飞船并配置装备。
  - 步骤 2：保存方案（使 isDirty = false）。
  - 步骤 3：修改装备配置（使 isDirty = true）。
  - 步骤 4：点击”新建”按钮。
  - 步骤 5：在 SmartSaveDialog 中选择”覆盖并新建”（discard）。
  - 步骤 6：断言飞船选择已清空（返回选船列表状态或显示无选中飞船）。
  - 步骤 7：断言筛选条件已清空（class, race, type 未选中）。
  - 步骤 8：断言蓝图配置已清空（blueprint = null）。

- [ ] 3.14 Bug修复验证 BUG-SBS-003（切换飞船未清空配装选中状态）
  - **Bug现状**: 点击”更换飞船”后选择新飞船时，原来飞船的装备仍然显示为选中状态。
  - 步骤 1：进入ship-build，选择大太刀并配置引擎装备。
  - 步骤 2：点击”更换飞船”按钮。
