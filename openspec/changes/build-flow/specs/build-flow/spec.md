...
**前提** 某个分组容器已渲染
**当** 用户查看该分组容器
**那么** 产线 cards 必须在左列竖向排列
**并且** 产出区 card 必须在右列

#### Scenario: 组内产线 card 拖拽排序

**前提** 某个分组内存在多条产线 card
**当** 用户拖拽某条产线 card
**那么** 该 card 可以在组内左列竖向列表中调整位置
**并且** 拖拽排序仅影响显示顺序，不改变逻辑数据

#### Scenario: 产线 card 初始位置

**前提** 分组推导完成
**当** 系统首次渲染分组内的产线 cards
**那么** 产线 cards 的初始位置为组内产线的默认顺序
