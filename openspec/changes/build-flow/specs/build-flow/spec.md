...
**前提** 系统已完成分组推导
**当** 系统渲染每个分组的产出区
**那么** 每个分组必须拥有两个独立的产出区 card：建材区（副标题"产出建材"）和材料区（副标题"产出材料"）
**并且** 建材区标签内容为该组内所有产线 sourceTags 的去重并集
**并且** 材料区标签内容为该组内所有产线 sourceTags 的去重并集
**并且** 两个 card 的标签 wareId 集合相同，但各自独立作为连线目标
...
**并且** 分组容器之间竖向排列
**并且** 每个分组容器内包含该组的产线 cards 和产出区双 card（建材区 + 材料区）
...
**前提** 某个分组存在一条或多条入选产线
**当** 系统渲染该分组的产出区
**那么** 建材区 card 必须显示"产出建材"副标题和标签
**并且** 材料区 card 必须显示"产出材料"副标题和标签
**并且** 两个 card 的标签内容均为该分组内入选产线"产线原材料"的去重并集
...
**前提** 产出区 card 已渲染"产出建材"或"产出材料"标签
**当** 用户查看标签布局
**那么** 标签文字必须右对齐
**并且** `+` 按钮必须常驻显示在标签左侧
**并且** 按钮必须从 card 左边外伸
...
**当** 系统计算可投放目标
**那么** 只允许匹配同组内 `wareId` 相同的"产线建材"、"产出建材"或"产出材料"目标标签
**并且** 不得按本地化名称文本做模糊匹配
**并且** 跨组目标不得出现在可投放列表中
...
**并且** 菜单项必须仅显示同组内目标
**并且** 菜单项必须显示目标产线名称或"产出建材"/"产出材料"，后跟 ware 名称作为辅助说明
**当** 用户从菜单中选择一个目标
**那么** 系统必须写入与拖拽相同的关系记录
...
**并且** 若 `targetType === 'output-build-material'` 且建材区的产出建材中不再包含该 `wareId`，该 assignment 必须被删除
**并且** 若 `targetType === 'output-material'` 且材料区的产出材料中不再包含该 `wareId`，该 assignment 必须被删除
...
#### Scenario: 删除产线时清理归档记录

**前提** 某条产线已被归档
**当** 用户在规划区删除该产线
**那么** 该产线的 groupId 必须从 `archivedGroupIds` 中移除
**并且** 归档计数必须同步减少

#### Scenario: 清空所有产线时清理归档记录

**前提** 存在已归档的产线
**当** 用户清空所有产线组
**那么** `archivedGroupIds` 必须被清空为空数组
**并且** 归档计数必须归零

### Requirement: Build Flow Virtual Default Connections

#### Scenario: 建材区默认连线

**前提** 某个分组建材区的 wareId 标签未被用户手动绑定
**当** 系统执行推导重算
**那么** 系统必须在该分组内非归档产线中，按 logic-flow 顺序搜索第一个 sourceTags 包含该 wareId 的产线
**并且** 若找到匹配产线，必须以虚拟连线记录该关系
**并且** 连线必须以虚线渲染
**并且** 标签底色必须按 wareId 分配颜色
**并且** 若未找到匹配产线，标签必须保持透明

#### Scenario: 材料区默认连线优先搜索归档产线

**前提** 某个分组材料区的 wareId 标签未被用户手动绑定
**当** 系统执行推导重算
**那么** 系统必须先在所有已归档产线中全局搜索第一个 sourceTags 包含该 wareId 的产线（按 logic-flow 顺序）
**并且** 若找到匹配的归档产线，标签必须涂色
**并且** 但不得绘制任何连线
**并且** 若未找到匹配的归档产线，再在本分组内非归档产线中搜索第一个匹配产线
**并且** 若找到非归档产线，必须以虚线连线并涂色
**并且** 若两步均未找到匹配产线，标签必须保持透明

#### Scenario: 默认连线不持久化

**前提** 系统已生成默认连线
**当** 用户保存当前方案
**那么** 默认连线不得写入 `BuildFlowPlanData`
**并且** 仅手动绑定的 assignments 被持久化
**当** 用户加载已保存方案
**那么** 默认连线在推导重算时重新生成
...
#### Scenario: 归档恢复后默认连线重算

**前提** 某条产线从归档状态恢复
**当** 系统执行推导重算
**那么** 材料区默认连线必须重新评估
**并且** 若恢复的产线在归档搜索中成为新的匹配，默认连线自动切换

#### Scenario: 默认连线颜色与手动连线一致

**前提** 某 wareId 同时存在默认连线和手动连线
**当** 系统分配颜色
**那么** 默认连线颜色必须与手动连线使用同一调色板
**并且** 按 wareId 排序后的 index % 8 分配
