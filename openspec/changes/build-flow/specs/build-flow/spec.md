...
### Requirement: Build Flow Line Archive

#### Scenario: 归档产线数据存储

**前提** 用户归档某条建筑产线
**当** 系统保存归档状态
**那么** 归档产线的 groupId 必须写入 `LogicFlowPlan.buildFlow.archivedGroupIds`
**并且** 归档产线数据必须随方案一起持久化

#### Scenario: 归档产线排除需求原材料计算

**前提** 某条产线已被归档
**当** 系统计算全局"需求原材料"
**那么** 归档产线的 buildCost 必须被排除
**并且** 归档产线不得参与建筑材料需求汇总

#### Scenario: 归档产线不显示在建筑产线区

**前提** 某条产线已被归档
**当** 系统渲染建筑产线区
**那么** 归档产线不得显示在产线 card 列表中
**并且** 归档产线不参与分组推导

#### Scenario: 归档时清理相关 assignments

**前提** 某条产线存在建筑流 assignments（作为来源或目标）
**当** 用户归档该产线
**那么** 该产线作为来源或目标的所有 assignments 必须被删除
**并且** 对应连线必须同步移除
...
**前提** 建筑产线区存在非归档产线
**当** 系统渲染产线 card
**那么** 每张产线 card 右上角必须显示 archive 图标
**并且** 点击图标触发归档操作

#### Scenario: 标题栏归档图标显示条件

**前提** 建筑产线区标题栏已渲染
**当** 存在至少一条归档产线
**那么** 标题栏右侧必须显示 archive 图标
**当** 无归档产线
**那么** 标题栏 archive 图标必须隐藏

#### Scenario: 点击标题栏归档图标弹出 Modal

**前提** 存在归档产线
**当** 用户点击标题栏 archive 图标
**那么** 系统必须弹出 Modal 显示已归档产线列表
**并且** Modal 内每条归档产线显示为简化版 card

#### Scenario: 恢复归档产线

**前提** Modal 显示归档产线列表
**当** 用户点击某条归档产线 card 右上角的恢复图标
**那么** 该产线必须从归档列表移除
**并且** 该产线重新参与建筑产线区计算与显示
**并且** Modal 内该产线 card 消失

#### Scenario: 全部恢复后 Modal 自动关闭
...
#### Scenario: 加载方案时恢复归档状态

**前提** 用户加载一个包含 `buildFlow.archivedGroupIds` 的方案
**当** 系统应用该方案
**那么** 归档产线列表必须从 `archivedGroupIds` 恢复
**并且** 归档产线不显示在建筑产线区

#### Scenario: 旧版方案无 archivedGroupIds 字段

**前提** 用户加载一个不含 `archivedGroupIds` 字段的旧版方案
**当** 系统读取该方案
**那么** 运行时归档列表必须初始化为空数组
**并且** 不得因缺少 `archivedGroupIds` 字段而报错
