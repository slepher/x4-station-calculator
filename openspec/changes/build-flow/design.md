...
建筑产线区按分组渲染，每个分组是一个带边框/背景的容器。分组容器之间横向排列（`flex` / `flex-row`）。
...
组内产线 card 支持拖拽调整竖向排列顺序（使用 `vuedraggable` 或等价方案）。产线 card 的初始位置为组内产线的默认顺序。拖拽排序仅影响显示顺序，不改变 `BuildFlowGroup.lineCards` 的逻辑顺序。
