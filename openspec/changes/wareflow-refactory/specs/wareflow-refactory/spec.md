...
将 WareFlow 计算拆分为两阶段架构：一阶段在 Store 层计算基础产量数据，二阶段在 Vue 层计算衍生数据（分组、资金、仓储、运输）并生成基础设施模块。

## ADDED Requirements

### Requirement: WareProductionFlow 数据类型

一阶段输出的新数据类型，仅包含产量相关字段。
...
Store 层调用的产量计算函数，位于 `src/store/logic/`。
...
Vue composable 调用的衍生数据计算函数，位于 `src/store/logic/`。
...
一阶段计算在 plannedModules 更新时直接调用，不是 watch。
...
二阶段通过 emit 更新 store 的 autoInfrastructureModules。
