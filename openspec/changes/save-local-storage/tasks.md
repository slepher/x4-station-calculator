...
## 2. Save 目录状态建模
- [x] 新增 Save 目录状态类型，定义 `version / activeArchiveId / list / settings`
- [x] 为 Save 目录状态补充默认空状态创建逻辑
- [x] 预留 Save 目录状态迁移入口，至少支持当前版本 schema 的安全加载
- [x] 将 save 地图 checkbox 设置建模为当前版本作用域下的 settings，而非 archive 局部状态
...
- [x] 改造 `clearAll()`，只清空当前版本作用域下的 Save 存档目录与正文，并保留 settings
- [x] 保持现有 `parser_version / post_processor_version / isCompatible / isValid` 语义不变
- [x] 让存档面板 checkbox 直接读写 Save Store settings，并在关闭面板或切换 archive 时保持

## 6. 地图入口行为
- [x] 进入 `maps` 视图时，在存在激活存档的情况下直接按该存档渲染地图
- [x] 将地图按存档渲染与存档面板开关解耦，避免自动展开面板
