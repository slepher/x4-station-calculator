...
- [x] 8.1 经济视图的数据源不再使用profitBreakdown, 而是使用wareFlowList, 每个WareFlowItem包含netValue, 正数为收入, 负数为支出, 生成wareFlowList的时候传入必要的数据
- [x] 8.2 将经济视图像仓储视图一样, 将产物分为三组, 三组分别为产品收入, 运营支出, 资源支出
- [x] 8.3 支出是运营支出还是资源支出取决于transportType, 不是container的就是资源支出
- [x] 8.4 分组从上往下排列分别是产品收入, 运营支出, 资源支出
- [x] 8.5 分组内容替换掉原先的列表展示
- [x] 8.6 每组的标题栏分别总和计算组内内容的netValue求和, 正数标绿, 负数标红
- [x] 8.7 分组的布局同仓储视图一致
- [x] 8.8 移除经济视图下方总产值和运营支出的显示, 保留profitTotal的显示,
- [x] 8.9 profitTotal需要从wareFlowList中计算, 计算方式为所有WareFlowItem的netValue求和
- [x] 8.10 profitTotal的样式需要从StationProfit的样式取回
- [x] 8.11 新增的文本要进行i18n, 分别为"资源支出", "运营支出", "产品收入"
- [x] 8.12 统一三个视图（数量、体积、经济）的层级结构，使它们成为同级元素
- [x] 8.13 移除经济视图的重复实现，保留单一的实现
- [x] 8.14 将经济视图明细显示在与数量视图相同的位置
- [x] 8.15 更新注释以准确描述各视图的功能

## 9. 资源视图的改动

- [x] 9.1 确认资源视图的数据源现在和netProduction无关, 而是使用wareFlowList
- [x] 9.2 如果还没有关联到wareFlowList, 则关联到wareFlowList
- [x] 9.3 资源视图现在使用和经济视图一样的分组方式, 如果代码复用经济视图, 则调用经济视图, 如有必要, 修改经济视图的代码以适应资源视图的分组方式
- [x] 9.4 资源视图的三个分组分别叫产品, 运营, 资源, 没有和经济视图一样的汇总(因为不同种类的产品产量不能叠加)
- [x] 9.5 确保经济视图的显示内容和当前保持一致
- [x] 9.6 产品/资源视图的分组内部的排序需要和allIndustryModules中对应模块的顺序相同, 如果模块有多个, 以第一个出现的模块为准
