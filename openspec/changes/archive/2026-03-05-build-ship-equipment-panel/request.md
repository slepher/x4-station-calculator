...
### 9.1 需求背景
当飞船槽位很多时（如驱逐舰、战列舰），所有 group 会挤在一起显示，超出合理范围。需要优化排列逻辑。

### 9.2 触发条件
- 仅在**标准模式（非简化模式）**下生效
- 简化模式（showAllSlots=false）保持原有紧凑排列
- 条件：所有 group 位置数量 > 8

### 9.3 排列规则
1. **第一层分组**：按 size 分为 N 行
   - 优先级：extralarge > large > medium > small
2. **第二层分组**：在每个 size 行内
   - 如果某个 group 的 count > 8，将其内部数量**平分**为两行显示
3. **位置计算**：
   - 简化模式：保持原有紧凑排列
   - 标准模式：按上述规则重新分组

### 9.4 示例
假设阿斯加德有 10 个 weapon 槽：
- 1 个 extralarge（count=1）
- 2 个 large（count=1 each）
- 多个 small/medium
