...
6. 点击分类子菜单，侧边栏切换为坐标列表（按星区分组），面包屑显示"存档 → 存档名 → 分类名"
7. 坐标列表有搜索框，输入文字可筛选星区名称
8. 点击坐标项，地图平移+缩放使兴趣点居中显示
9. 每个兴趣点标记上方显示该实体的 `code` 字段
10. 点击地图上兴趣点，显示 tooltip（包含分类名、owner、坐标等基本信息）
11. 面包屑导航可点击返回上一层
12. 各分类使用不同颜色标记区分
13. UI 风格与地图现有 amber/深色主题一致
