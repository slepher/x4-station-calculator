...
系统 MUST 在 Step 3 显示 coverage 派生的 save stations 与用户创建的 virtual stations。

#### Scenario: 用户进入某个 group 的 Step 3
- **前提** group 已设置 coverage
- **当** 用户进入 Step 3
- **那么** 系统 SHALL 按 coverage 自动列出当前 archive 中的 save stations
- **并且** SHALL 显示已有 virtual station plans
...
- **前提** 用户在地图 binding 面板中修改了 group 或 station planning
- **当** 改动尚未保存
