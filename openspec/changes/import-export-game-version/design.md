...
### 4.5 Ship 装备槽位兼容校验
- 仅靠 `equipment_id` 是否存在，无法覆盖“装备仍然存在，但已经不再匹配当前舰船槽位”的情况。
- 本次 Ship 清洗收敛到一条更直接的规则：
  - 当装备仍存在时，继续校验它是否匹配当前舰船对应 group 的 `slot_type / size / tags`
- 兼容性判定规则与飞船装配候选逻辑保持一致：
  - `type` 必须匹配
  - `size` 必须匹配
  - `equipment.slotTags` 必须被当前连接 `tags` 完整覆盖
- 若不匹配，则在导入前清空该装备引用，并计入 Ship 模块清洗摘要。
- 这样可覆盖跨版本常见风险：
  - 装备仍存在，但当前舰船该槽位规则已变化
  - 装备仍存在，但当前版本不再允许装到这条连接上
