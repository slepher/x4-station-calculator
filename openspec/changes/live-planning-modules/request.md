# Live Planning Modules

## 目标

修正 `live-planning-modules` 方案中的语义冲突，明确 `recommendedModules` 并不是“待采纳建议”，而是已经纳入 planning 基线的一部分，并直接显示在 planning 区中，只通过视觉样式区分来源。
...
- UI 中不再作为 planning 区之外的独立推荐区存在，而是显示在 planning 区内部的 inline 子区块中。
...
- recommended 模块显示在 planned 区中，作为其中的 inline 子区块呈现，不再作为 planning 区之外的独立区块。
...
- recommended 项与普通 planned 项同属 planning 区交互语义；展示上保留 inline 子区块，点击时执行“转正到当前目标总量”。
...
- 同步 `request/spec/design/tasks`，避免继续依赖旧的冲突语义
...
3. recommended 模块显示在 planning 区中，并通过 inline 子区块与虚线前置等视觉样式区分来源。
