...
- [x] 4. 扩展 transit transport presenter
  - [x] 4.1 接入 ship-build 蓝图、ship、equipment 数据源
  - [x] 4.2 输出左侧运输船候选分组、空态、未选择提示状态
  - [x] 4.3 校验当前 selected blueprint 是否仍可用；不可用时提供清空信号
  - [x] 4.4 为 Sector Group row 和 segments 增加嵌套 `travel`
  - [x] 4.5 为 Station sector group 增加 `travel` 与 `hideSectorHeader`
  - [x] 4.6 为 Station row 增加 local/total/throughput `travel`

- [x] 5. 实现左侧运输船选择 UI
  - [x] 5.1 在 transit hub 建筑区下方接入选择组件或区域
  - [x] 5.2 渲染无候选提示与前往 ship-build 操作
  - [x] 5.3 渲染有候选未选择提示
  - [x] 5.4 渲染飞船分组、蓝图行和参数 chips
  - [x] 5.5 选择蓝图时调用 `useLiveProductionStore` action

- [x] 6. 扩展右侧运输路线 UI
  - [x] 6.1 Sector Group 摘要渲染耗时和单程吞吐量 metric chip
  - [x] 6.2 Sector Group 明细普通空间段渲染耗时列
  - [x] 6.3 Station 同星区隐藏 sector header，仅渲染 station row 估算
  - [x] 6.4 Station 跨星区 sector row 渲染耗时，明细渲染普通空间段耗时
  - [x] 6.5 Station row 渲染星区内耗时、总耗时、单程吞吐量
  - [x] 6.6 未选择运输船时不渲染新增耗时/吞吐量 UI

- [x] 7. i18n 文案
  - [x] 7.1 更新 `src/locales/en.json`
  - [x] 7.2 更新 `src/locales/zh-CN.json`
  - [x] 7.3 覆盖选择区、chips、耗时、单程吞吐量、空态与前往 ship-build 文案
...
- 不修改 Rust parser。
- 不实现 highway/superhighway 耗时或路径规划。
